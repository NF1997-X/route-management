  ...
  return (
    <div className="relative inline-block">
      ...
      {isVisible && (
        ...
        >
          <div className="px-3 py-2 text-xs bg-popover text-popover-foreground rounded-lg shadow-md whitespace-nowrap border border-border">
            {content}
          </div>
        ...
        <div className="flex items-center gap-2 flex-shrink-0">
          ...
          <Popover>
            <PopoverTrigger asChild>
              <Button 
                variant="outline" 
                className="h-8 w-8 p-0 pagination-button rounded-lg border border-border"
                data-testid="sort-trigger"
              >
                {sortState ? (
                  sortState.direction === 'asc' ? <ArrowUp className="w-4 h-4 text-muted-foreground" /> : <ArrowDown className="w-4 h-4 text-muted-foreground" />
                ) : (
                  <ArrowUpDown className="w-4 h-4 opacity-50 text-muted-foreground" />
                )}
              </Button>
            </PopoverTrigger>
      ...
      {(searchTerm || filterValue.length > 0 || deliveryFilterValue.length > 0) && (
        <div className="px-6 py-2 border-b border-border bg-muted">
          <div className="flex flex-wrap items-center gap-1">
            <span className="text-muted-foreground font-semibold text-xs">Active:</span>
            {searchTerm && (
            ...
            {filterValue.map(route => {
              ...
              return (
                ...
                >
                  ...
                  <span className="text-[8px] font-semibold text-muted-foreground">{route}</span>
                </button>
              );
            })}
              ...
              >
                {(provided) => (
                  ...
                  >
                      ...
                      >
                        {(provided) => (
                          <TableHead
                            ...
                            className="px-4 py-2 text-center table-header-footer-12px font-semibold tracking-wide sticky top-0 whitespace-nowrap border-transparent text-foreground"
                            style={{
                              textAlign: "center",
                    ...
                    <TableHead
                      className="p-0 py-2 text-center table-header-footer-12px font-semibold tracking-wide sticky top-0 whitespace-nowrap border-transparent"
                      style={{
                        textAlign: "center",
                        textDecoration: "normal",
                        fontSize: "10px",
                  ...
                  {isLoading
                    ? Array.from({ length: Math.min(pageSize, 5) }).map(
                        (_, index) => (
                          <tr
                            key={`skeleton-${index}`}
                            className={`skeleton-row border-b border-border shadow-md ${
                              index % 2 === 0 
                                ? 'bg-card' 
                        ...
                        >
                          {(provided, snapshot) => (
                            <TableRow
                              ...
                              } hover:bg-accent hover:shadow-md transition-all duration-300 hover:-translate-y-0.5 border-b border-border ${
                            ...
                            >
                              {visibleColumns.map((column) => (
                                <TableCell
                                  key={column.id}
                                  className="p-4 align-middle [&:has([role=checkbox])]:pr-0 px-4 py-2 table-cell-10px text-center text-[9px] text-slate-800 dark:text-slate-200 font-semibold whitespace-nowrap"
                                  style={{
                                    minWidth: "100px",
                                    ...(column.dataKey === "location" && {
                                      minWidth: `${120 + 15}px`,
              ...
              <div className="flex items-center justify-between gap-1 w-full">
                ...
                {totalPages > 5 && currentPage > 3 && (
                  <Button
                    ...
                    size="xs"
                    onClick={() => goToPage(1)}
                    className="pagination-button h-5 w-5 p-0"
                    data-testid="button-first-page"
                  >
                    <ChevronsLeft className="h-2 w-2 text-gray-400 dark:text-current" />
                  </Button>
                )}
                      ...
                      return pages.map((pageNum) => {
                        ...
                        return (
                          <Button
                            ...
                            className={`pagination-button page-number min-w-[20px] h-5 text-[8px] ${
                ...
                {totalPages > 5 && currentPage < totalPages - 2 && (
                  ...
                  >
                    <ChevronsRight className="h-2 w-2 text-gray-400 dark:text-current" />
                  </Button>
                )}
                </div>
              </div>
